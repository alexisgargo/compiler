...
class Parser
{
  ...
  int current_token()
  {
    ...
    {
      return -1; // End of input (equivalent to $)
    }
    return tokens[position];
  }

  // Function to consume a token
  bool consume(int expected_token)
  {
  ...
  bool S()
  {
    // std::cout << "Parsing S at position " << position << std::endl;
    ...
    // Try S -> OOP
    // FIRST+(S -> OOP) = {2, 3}
    if (current_token() == 2 || current_token() == 3)
    {
      if (OOP())
      {
        std::cout << "S -> OOP successful" << std::endl;
        return true;
      }
    }

    // Backtrack and try S -> PP
    position = saved_pos;
    ...
    if (current_token() == 1)
    {
      if (PP())
      {
        std::cout << "S -> PP successful" << std::endl;
        return true;
      }
    }

    std::cout << "S parsing failed" << std::endl;
    return false;
  }

  // Grammar: OOP -> <2> COMP | <3> COMP
  bool OOP()
  {
    std::cout << "Parsing OOP at position " << position << std::endl;
    ...
    if (current_token() == 2)
    {
      if (consume(2) && COMP())
      {
        std::cout << "OOP -> <2> COMP successful" << std::endl;
        return true;
      }
      std::cout << "OOP -> <2> COMP failed" << std::endl;
      return false;
    }
    ...
    if (current_token() == 3)
    {
      if (consume(3) && COMP())
      {
        std::cout << "OOP -> <3> COMP successful" << std::endl;
        return true;
      }
      std::cout << "OOP -> <3> COMP failed" << std::endl;
      return false;
    }

    std::cout << "OOP parsing failed" << std::endl;
    return false;
  }
  ...
  bool PP()
  {
    std::cout << "Parsing PP at position " << position << std::endl;
    ...
    if (current_token() == 1)
    {
      if (consume(1) && COMP())
      {
        std::cout << "PP -> <1> COMP successful" << std::endl;
        return true;
      }
      std::cout << "PP -> <1> COMP failed" << std::endl;
      return false;
    }

    std::cout << "PP parsing failed" << std::endl;
    return false;
  }

  // Grammar: COMP -> PP | OOP | e
  bool COMP()
  {
    std::cout << "Parsing COMP at position " << position << std::endl;
    ...
    if (current_token() == 1)
    {
      if (PP())
      {
        std::cout << "COMP -> PP successful" << std::endl;
        return true;
      }
    }

    // Backtrack and try COMP -> OOP
    position = saved_pos;
    ...
    if (current_token() == 2 || current_token() == 3)
    {
      if (OOP())
      {
        std::cout << "COMP -> OOP successful" << std::endl;
        return true;
      }
    }

    // Backtrack and try COMP -> e (epsilon)
    position = saved_pos;

    // Try COMP -> e
    // FIRST+(COMP -> e) = {$} (end of input or FOLLOW set)
    std::cout << "COMP -> e (epsilon) successful" << std::endl;
    return true; // Epsilon production always succeeds
  }
  ...
  bool parse()
  {
    ...
    std::cout << "Starting parse with tokens: ";
    ...
    {
      std::cout << "\nParsing successful! All tokens consumed." << std::endl;
      return true;
    }
    else if (result)
    {
      std::cout << "\nParsing failed: Not all tokens were consumed. Stopped at position "
                << position << std::endl;
      return false;
    }
    else
...
// std::string S(std::vector<int> &tokens) {
//   if (tokens[0] == 1) OOP();
//   else PP();
// }
